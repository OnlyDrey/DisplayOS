...
echo "[+] Preparing live-build config tree..."
...
mkdir -p \
  ...
  "${CONFIG_DIR}/includes.chroot/root/.config/xfce4/xfconf/xfce-perchannel-xml" \
  "${CONFIG_DIR}/includes.chroot/root/.config/autostart" \
...
# Assets
echo "[+] Copying assets..."
[ -f "${WALLPAPER_SRC}" ] && cp -f "${WALLPAPER_SRC}" "${CONFIG_DIR}/includes.chroot/usr/local/share/displayos/wallpaper.png" || true
[ -f "${WALLPAPER_SRC}" ] && cp -f "${WALLPAPER_SRC}" "${CONFIG_DIR}/includes.chroot/boot/grub/splash.png" || true
...
cat > "${CONFIG_DIR}/includes.chroot/root/.config/xfce4/xfconf/xfce-perchannel-xml/xfwm4.xml" <<'EOF'
